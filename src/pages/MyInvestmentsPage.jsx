      ...
      {stats && stats.activeLending > 0 && (
        <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 mb-6 rounded">
          <div className="flex items-start">
            ...
            <div>
              <h3 className="text-sm font-semibold text-yellow-800 mb-1">Diversification Suggestion</h3>
              <p className="text-sm text-yellow-700">
                Consider diversifying your portfolio across different risk levels to optimize returns and reduce overall risk exposure.
              </p>
            </div>
          </div>
        </div>
      ...
      {stats && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          <div className="bg-white rounded-xl shadow-lg p-6">
            <div className="text-sm font-semibold text-gray-600 mb-1">Total Invested</div>
            <div className="text-3xl font-bold text-gray-900">${stats.totalLent.toLocaleString()}</div>
          </div>
          <div className="bg-white rounded-xl shadow-lg p-6">
            <div className="text-sm font-semibold text-gray-600 mb-1">Active Investments</div>
            <div className="text-3xl font-bold text-blue-600">{stats.activeLending}</div>
          </div>
          <div className="bg-white rounded-xl shadow-lg p-6">
            <div className="text-sm font-semibold text-gray-600 mb-1">Returns Earned</div>
            <div className="text-3xl font-bold text-green-600">${stats.totalReturnsEarned.toLocaleString()}</div>
          </div>
          <div className="bg-white rounded-xl shadow-lg p-6">
            <div className="text-sm font-semibold text-gray-600 mb-1">Completed</div>
            <div className="text-3xl font-bold text-purple-600">{stats.completedLoans}</div>
          </div>
        </div>
      )}
      ...
      <div className="bg-white rounded-xl shadow-lg p-6">
        ...
        {filteredInvestments.length === 0 ? (
          <div className="text-center py-12 text-gray-500">
            ...
            <p className="text-sm mb-4">Start investing by exploring available borrowers</p>
            <button
              onClick={() => navigate('/marketplace/explore')}
              className="bg-purple-600 text-white px-6 py-2 rounded-lg hover:bg-purple-700"
            >
          ...
          <div className="space-y-4">
            {filteredInvestments.map((investment) => (
              <div
                ...
                onClick={() => navigate(`/marketplace/investment/${investment.loanId}`)}
              >
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <span className={`px-3 py-1 rounded-full text-sm font-semibold ${getRiskColor(investment.riskLevel)}`}>
                        {investment.riskLevel} Risk
                      </span>
                    </div>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm text-gray-600">
                      <div>
                        <span className="font-semibold">Amount:</span> ${investment.amount.toLocaleString()}
                      </div>
                      <div>
                        <span className="font-semibold">Interest Rate:</span> {investment.interestRate}%
                      </div>
                      <div>
                        <span className="font-semibold">Expected Return:</span> ${calculateExpectedReturn(investment).toLocaleString()}
                      </div>
                      <div>
                        <span className="font-semibold">Type:</span> {investment.trustType === 'AI' ? 'AI Verified' : 'Social Trust'}
                      </div>
                    </div>
                    <div className="text-xs text-gray-500 mt-2">
                      Created: {investment.createdAt}
                      {investment.completedAt && ` • Completed: ${investment.completedAt}`}
                    </div>
                  </div>
