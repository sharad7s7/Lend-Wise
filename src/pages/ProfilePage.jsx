  ...
  useEffect(() => {
    if (user?.id) {
      const fetchData = async () => {
        try {
          const borrowing = await loanService.getBorrowingHistory(user.id);
          const lending = await loanService.getLendingHistory(user.id);
          const requests = await borrowRequestService.getUserRequests(user.id);
          // Mock deposit info since API endpoint is missing
          const depositInfo = { amount: 0, status: "None" };
  ...
  const content = (
    <div className="container mx-auto px-4 py-8">
          ...
          <div className="space-y-4">
            {user?.role === "Student" && user?.socialTrustScore !== null && (
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Based on Friend Circle activity
                </p>
              </div>
            )}
            {(user?.role === "Non-student" || user?.role === "Lender") &&
              user?.aiCreditScore !== null && (
                <>
                  <div>
                    <label className="text-sm font-semibold text-gray-600">
                      AI Credit Score
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      Based on marketplace assessment
                    </p>
                  </div>
                  <div>
                    <label className="text-sm font-semibold text-gray-600">
            ...
            {!user?.socialTrustScore && !user?.aiCreditScore && (
              <p className="text-gray-500">
                Complete your first transaction to see your trust score
              </p>
            )}
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl shadow-lg p-6 mb-8 border border-green-200">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            ...
            <div className="bg-white rounded-lg p-4 border border-emerald-200">
              ...
              <p className="text-xs text-gray-500 mt-1">
                Can be deducted if you default
              </p>
            </div>
            ...
            <div className="bg-white rounded-lg p-4 border border-orange-200">
              ...
              <div
                className={`text-3xl font-bold mt-2 ${
                  securityDepositInfo.deductedAmount > 0
                    ? "text-orange-600"
                    : "text-gray-600"
                }`}
              >
              ...
              <p className="text-xs text-gray-500 mt-1">
                From defaults or penalties
              </p>
            </div>
          </div>

          {/* Security Deposit History */}
          {securityDepositInfo.history &&
            securityDepositInfo.history.length > 0 && (
              <div className="mt-6 pt-6 border-t border-green-200">
                ...
                <div className="space-y-2 max-h-40 overflow-y-auto">
                  {securityDepositInfo.history.map((transaction, idx) => (
                    <div
                      key={idx}
                      className="flex items-center justify-between text-sm bg-white p-3 rounded border border-gray-100"
                    >
          ...
          <div className="mt-6 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
            <p className="text-sm text-yellow-800">
              <strong>⚠️ Important:</strong> Your security deposit will be
              deducted if you fail to repay a loan within the deadline. Keep
              your available balance positive to protect your borrowing
              reputation.
            </p>
          </div>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
        ...
        myRequests.filter((r) => r.status === "Funded").length === 0 ? (
              ...
              <tbody>
                {borrowingHistory.map((loan) => (
                  ...
                  >
                    ...
                    <td className="py-3 px-4">
                      {loan.interestRate > 0 ? `${loan.interestRate}%` : "0%"}
                    </td>
                ...
                {lendingHistory.map((loan) => {
                  const expectedReturn =
                    loan.status === "Completed"
                      ? loan.interestPaid || 0
                      : Math.round(
                          (((loan.amount * (loan.interestRate || 0)) / 100) *
                            (loan.duration || 12)) /
                            12
                        );
